...
//				ApplyOrders(world, xy, mi);
			...
			{
                var newSelection2 = SelectActorsInBox(world, dragStart, xy, _ => true);
                if (newSelection2.ToArray().Length == 1)
                {
                    if (/*world.Selection.Contains(newSelection2.ElementAt(0)) &&*/ (world.Selection.Actors.ToArray().Length == 1))
                    {
 //                       Game.Debug("Selection length = {0}", world.Selection.Actors.ToArray().Length);
                        ApplyOrders(world, xy, mi);
                    }
                    else
                    {
                        world.Selection.Combine(world, newSelection2, mi.Modifiers.HasModifier(Modifiers.Shift), dragStart == xy);
                    }
                    LoseFocus(mi);
                    return true;
                }
            ...
            {
                world.Selection.Clear(); // empty selection
            }
